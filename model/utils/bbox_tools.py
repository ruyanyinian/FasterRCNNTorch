...
def generate_anchor_base(base_size=16, ratios=[0.5, 1, 2],
  ...
  for i in six.moves.range(len(ratios)):  # ratios = [0,1,2]
    for j in six.moves.range(len(anchor_scales)): # anchor_scales = [8, 16, 32]
      # 生成9种不同比例的h和w
      h = base_size * anchor_scales[j] * np.sqrt(ratios[i])
      w = base_size * anchor_scales[j] * np.sqrt(1. / ratios[i])

      index = i * len(anchor_scales) + j
      # 计算出anchor_base画的9个框的左下角和右上角的4个anchor坐标值(也就是xmin, ymin, xmax, ymax)
